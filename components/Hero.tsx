...
export function Hero() {
    return <section id="top" className="hero-section page-wrap">
        <div className="hero-copy">
            ...
            <h1><AnimatedText lines={["Your brand deserves", <em key="accent">to stand out.</em>]} delay={0.1} /></h1>
        ...
        <motion.div className="studio-interface" initial={{ opacity: 0, y: 30, rotate: 1 }} animate={{ opacity: 1, y: 0, rotate: 0 }} transition={{ duration: 1, delay: 0.8 }}>
            ...
            <div className="interface-list">{["WEB DEV", "BRAND", "CONTENT", "SOCIAL", "MARKETING"].map((item, i) => <div key={item}><span>0{i + 1}</span><strong>{item}</strong><span className="interface-dot" /></div>)}</div>
